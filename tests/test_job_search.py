...
def test_collect_replay_seed_leads_defers_stale_posted_hint_for_recently_revalidated_history_jobs(tmp_path: Path) -> None:
    settings = build_settings()
    settings.data_dir = tmp_path
    stale_posted_date = (date.today() - timedelta(days=settings.posted_within_days + 5)).isoformat()
                ...
                "bundles": [
                    {
                        "job": {
                            ...
                            "resolved_job_url": "https://portal.dynamicsats.com/JobListing/Details/be9c621a-ba9d-41b6-bc7b-917d59117a03/eed50803-efca-f011-bbd3-6045bdeb7e04",
                            "ats_platform": "DynamicsATS",
                            "location_text": "United States",
                            "is_fully_remote": True,
                            "posted_date_text": stale_posted_date,
                            "posted_date_iso": stale_posted_date,
                            "base_salary_min_usd": 165000,
                            "base_salary_max_usd": 220000,
                            "salary_text": "$165,000 - $220,000",
                            "evidence_notes": "Historical accepted role.",
                            "validation_evidence": [],
    ...
    assert len(leads) == 1
    assert leads[0].company_name == "Quorum Software"
    assert leads[0].posted_date_hint is None
    assert "deferred stale posted-date hint" in leads[0].evidence_notes


def test_collect_replay_seed_leads_keeps_old_stale_history_jobs_suppressed_without_recent_revalidation(tmp_path: Path) -> None:
    settings = build_settings()
    settings.data_dir = tmp_path
    stale_posted_date = (date.today() - timedelta(days=settings.posted_within_days + 5)).isoformat()
    old_reported_at = f"{(date.today() - timedelta(days=settings.posted_within_days + 5)).isoformat()}T10:00:00+00:00"
    (settings.data_dir / "run-20260404-000000.json").write_text(
        json.dumps(
            {
                "bundles": [
                    {
                        "job": {
                            ...
                            "evidence_notes": "Historical accepted role.",
                            "validation_evidence": [],
                            "last_reported_at": old_reported_at,
                            "first_reported_at": old_reported_at,
                            "report_count": 12,
                        }
                    }
                ],
...
def test_collect_replay_seed_leads_defers_stale_curated_seed_hint_when_validated_history_is_recent(tmp_path: Path) -> None:
    settings = build_settings()
    settings.data_dir = tmp_path
    stale_posted_date = (date.today() - timedelta(days=settings.posted_within_days + 5)).isoformat()
    recent_reported_at = f"{date.today().isoformat()}T10:00:00+00:00"
    direct_job_url = "https://www.dynatrace.com/careers/jobs/1370163900"
    (settings.data_dir / "seed-leads.json").write_text(
        json.dumps(
            {
                "leads": [
                    {
                        "company_name": "Dynatrace",
                        "role_title": "Senior AI Product Manager",
                        "source_url": direct_job_url,
                        "source_type": "company_site",
                        "direct_job_url": direct_job_url,
                        "location_hint": "Remote",
                        "posted_date_hint": stale_posted_date,
                        "is_remote_hint": True,
                        "salary_text_hint": "$175K - $220K",
                        "evidence_notes": "Curated direct company seed.",
                    }
                ]
            }
        )
    ...
    assert len(leads) == 1
    assert leads[0].company_name == "Dynatrace"
    assert leads[0].posted_date_hint is None
    assert "deferred stale posted-date hint" in leads[0].evidence_notes
